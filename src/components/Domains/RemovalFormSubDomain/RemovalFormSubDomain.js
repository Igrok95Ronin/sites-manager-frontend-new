...
export default function RemovalFormSubDomain({
  subDomainToDelete,
  showRemovalFromSubDomain,
  setShowRemovalFromSubDomain,
  setLoading,
  setError,
  setOpen,
  onUpdateDomains,
  ...
  const handleClose = () => {
    setShowRemovalFromSubDomain(false);
  };
  ...
  const request = async (securityPassword) => {
    try {
      ...
      // });
      await axios.post(
        `${APIURL}/deletesubdomain`,
        {
          subDomain: subDomainToDelete, // Название поддомена который удаляем
          securityPassword: securityPassword, // Отправляем пароль безопасности
        },
      );
      ...
      >
        ...
        <DialogContent>
          <DialogContentText className="removalForm__subTitle">
            Чтобы полностью удалить поддомен из БД и Cloudflare, необходимо
            ввести пароль безопасности.
          </DialogContentText>
          <TextField
            autoFocus
            required
